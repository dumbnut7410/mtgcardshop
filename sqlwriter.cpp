...
void SQLWriter::listEvents() {
  ...
  if (!ret)
    ...
    while (query.next()) {
      //get players
      std::vector<int> playerids = this->CSVParse(query.value(
                                     2).toString().toStdString());
...
bool SQLWriter::registerForEvent(std::string playerName, int eventId,
                                 int price) {
  QSqlQuery query;
  std::string command;
...
void SQLWriter::listPossibleItems() {
  ...
  while (query.next()) {
    std::cout << query.value(0).toString().toStdString() << "\t" << query.value(
                1).toString().toStdString() << std::endl;
  }
...
void SQLWriter::printelostandings() {
  ...
  while (query.next()) {
    ...
    std::string str = std::to_string(i) + ". " + query.value(
                        0).toString().toStdString();
    std::cout << str;
...
int SQLWriter::addItemToDB(QString str, int hide) {
  ...
  QString command =
    "INSERT INTO `products` (`id`, `name`, `hidden`) VALUES (NULL, '";
  command.append(str);
  command.append("',");
...
bool SQLWriter::addPlayer(Player p) {
  QSqlQuery query;
  std::string command =
    "INSERT INTO `players` (`id`, `name`, `elo`) VALUES (NULL, '";
...
int SQLWriter::getPlayerID(std::string name) {
  ...
  if (!query.next()) {
    std::cout << "player " << name << " not found, create?" << std::endl;
    std::string ans;
    if (std::cin >> ans) {
      if (ans == "y" || ans == "Y") {
        Player p;
...
/**
 * @brief decays elo of all players
 * @details subtracts amount from each players elo and pushes new elo back
 *
 * @param players list of players to decay
 * @param amount amount to decrease all players elo
 */
void SQLWriter::decayElo(std::string player, int amount) {
  int id = this->getPlayerID(player);
  int elo = this->getElo(player);
  elo -= amount;
  this->setElo(id, elo);
...
/**
 * @brief map of all players and their ids
 * @details map of name and id fields from the players table
 * @return map of players
 */
std::map<std::string, int> SQLWriter::getAllPlayers() {
  std::map<std::string, int> players;
  QSqlQuery query;
  QString command = "SELECT `name`, `id` FROM `players`";
  ...
  while (query.next()) {
    players[query.value(0).toString().toStdString()] = query.value(1).toInt();
  }
  return players;
}
/**
 ...
 * @param winner player1 win = 1, player2 win = 2, draw = 0
 * @return true if the operation was successful, false otherwise
 */
bool SQLWriter::changeElo(std::string player1, std::string player2,
                          int winner) {
  ...
  std::cout << "player 1 change: " << change << ", current : " << elo1 + change <<
            std::endl;
  std::cout << "player 2 change: " << -change << ", current : " << elo2 - change
            << std::endl;
